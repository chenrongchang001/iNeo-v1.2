...
#    RNA_Mut_lev_trans = Mut_lev_ABC(RNA_Mut_lev)
    ...
    I_SB_trans = ratio_trans(I_SB_ratio)
    II_SB_trans = ratio_trans(II_SB_ratio)
#calculate rank
...
#    temp_neo_score = MHC_I_weight*MHC_I_trans + Exp_weight*Exp_trans + AF_weight*AF_trans + MT_weight*Mut_num_trans + Mut_lev_weight*DNA_Mut_lev_trans*RNA_Mut_lev_trans
    MHC_I_quality = I_SB_trans * ratio_trans(I_AC_trans)
    MHC_II_quality = II_SB_trans * ratio_trans(II_AC_trans)
    ...
    temp_neo_score2_expression = f'DNA_AF:{str(round(DNA_AF_trans,3))}*RNA_AF:{str(round(RNA_AF_trans,3))}*M:{str(round(Mut_num_trans,3))}*RPKM:{str(round(Exp_trans,3))}*(log2(MHC_I:{str(num_mut_I_SB_WB_epitope)})={str(MHC_I_trans)})*(I_SB:{str(round(I_SB_trans,3))})(*I_AC:{str(round(ratio_trans(I_AC_trans),3))})={str(temp_neo_score2)}'
    if(num_mut_II_SB_WB_epitope == 0):
        MHC_II_trans = 0
    temp_bonus = 0.1*MHC_II_trans*MHC_II_quality
    temp_bonus_val = temp_bonus*temp_neo_score2
    temp_bounus_expression = f'0.1*(log2(MHC_II:{str(round(num_mut_II_SB_WB_epitope,3))}={str(MHC_II_trans)})*MHC_II_qual:(II_SB:{str(round(II_SB_trans,3))})*(II_AC:{str(round(ratio_trans(II_AC_trans),3))})={str(round(MHC_II_quality,3))})={str(temp_bonus)}'
#filter
#    if(float(DNA_AF_one) <= 0.0299 or float(RNA_AF_one) <= 0.002):
#        temp_neo_score = 0
#        temp_neo_score2 = 0
#        temp_bonus = 0
    if(Gene_name.startswith('HLA-') or Gene_name.startswith('IG')):
        temp_neo_score = 0
        temp_neo_score2 = 0
        temp_bonus = 0
