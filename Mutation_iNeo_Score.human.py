...
input = pd.DataFrame(pd.read_excel(Filename))
...
Is_WES_sample = 0
if(max(RNA_AF_array) == '.' or max(RNA_AF_array) == 0):
    Is_WES_sample = 1
output_data = pd.DataFrame(columns=['Chr','Position','Ref', 'Alt','Mut_type','AA_change','Gene_name','ENSG',
...
for i in range(len(input)):
    ...
    Is_driver_gene = line.Is_driver_gene 
...
#    RNA_Mut_lev_trans = Mut_lev_ABC(RNA_Mut_lev)
    ...
    I_SB_ratio,II_SB_ratio = 0,0
    if(num_mut_I_SB_WB_epitope != 0):
        I_SB_ratio = I_SB/num_mut_I_SB_WB_epitope
    if(num_mut_II_SB_WB_epitope != 0):
        II_SB_ratio = II_SB/num_mut_II_SB_WB_epitope
    I_SB_trans = I_SB_ratio
    II_SB_trans = II_SB_ratio
#calculate rank
    ...
    try:
        ...
        RNA_AF_rank = 1
    if(RNA_AF_one==0 and Is_WES_sample==1):
        RNA_AF_one = 1
        RNA_AF_rank = 1
        RNA_AF_trans = 1
...
#    temp_neo_score = MHC_I_weight*MHC_I_trans + Exp_weight*Exp_trans + AF_weight*AF_trans + MT_weight*Mut_num_trans + Mut_lev_weight*DNA_Mut_lev_trans*RNA_Mut_lev_trans
    MHC_I_quality = 1 + I_SB_trans + I_AC_trans
    MHC_II_quality = 1 + II_SB_trans + II_AC_trans
    MHC_I_trans = RPKM_alter(num_mut_I_SB_WB_epitope)
    MHC_II_trans = RPKM_alter(num_mut_II_SB_WB_epitope)
    temp_neo_score2 = MHC_I_trans*MHC_I_quality*Exp_trans*AF_trans*Mut_num_trans
    temp_neo_score2_expression = f'DNA_AF:{round(DNA_AF_trans,3)}*RNA_AF:{round(RNA_AF_trans,3)}*M:{round(Mut_num_trans,3)}*RPKM:{round(Exp_trans,3)}*(log2(MHC_I:{num_mut_I_SB_WB_epitope})={MHC_I_trans})*(1+I_SB:{round(I_SB_trans,3)}+I_AC:{round(ratio_trans(I_AC_trans),3)}={MHC_I_quality})={temp_neo_score2}'
#    temp_neo_score2_expression = ""
    if(num_mut_II_SB_WB_epitope == 0):
        MHC_II_trans = 0
    temp_bonus = 0.1*MHC_II_trans*MHC_II_quality+0.2*Is_driver_gene
    temp_bonus_val = temp_bonus*temp_neo_score2
    temp_bounus_expression = f'0.1*(log2(MHC_II:{round(num_mut_II_SB_WB_epitope,3)}={MHC_II_trans})*MHC_II_qual:(1+II_SB:{round(II_SB_trans,3)}+II_AC:{round(ratio_trans(II_AC_trans),3)})={round(MHC_II_quality,3)})+(0.2*{Is_driver_gene})={temp_bonus}'
#    temp_bounus_expression = ""
#filter
#    if(float(DNA_AF_one) <= 0.0299 or float(RNA_AF_one) <= 0.002):
#        temp_neo_score = 0
#        temp_neo_score2 = 0
#        temp_bonus = 0
    if(str(Gene_name).startswith('HLA-') or str(Gene_name).startswith('IG')):
        temp_neo_score = 0
        temp_neo_score2 = 0
        temp_bonus = 0
...
#put data into dataframe
    new = [Chr,Position,Ref,Alt,Mut_type,AA_change,Gene_name,ENSG,DNA_AF_one,DNA_AF_rank,DNA_AF_trans,RNA_AF_one,RNA_AF_rank,RNA_AF_trans,Mut_AA_num,
          ...
          MHC_II_rank,MHC_II_trans,Is_driver_gene,I_AC_A,I_AC_B,I_AC_C,I_AC,I_AC_trans,I_AC_rank,II_AC_A,II_AC_B,II_AC_C,II_AC,II_AC_trans,II_AC_rank,num_mut_I_SB_epitope,I_SB/num_mut_I_SB_WB_epitope,I_SB_rank,num_mut_II_SB_epitope,II_SB/num_mut_II_SB_WB_epitope,
